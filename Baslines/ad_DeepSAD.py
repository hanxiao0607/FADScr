from models import DeepSAD, SeqDeepSAD
...
def main():
    ...
    for i in lst:
        ...
        deepsad = DeepSAD.DeepSAD(input_dim=293, out_dim=128, max_epoch=20, batch_size=1024, nu=0.35)
        ...
        print('Anomaly Detection FPR-AT-95-TPR: {:.5f}'.format(
            utils.getfpr95tpr(y_true=test_y, dist=y_dist, steps=1000)))
    ...
    for i in lst:
        ...
        deepsad = DeepSAD.DeepSAD(input_dim=77, out_dim=128, batch_size=1024, max_epoch=20, nu=0.05)
        deepsad.train_DeepSAD(seen_x, seen_y, unseen_x)
        deepsad.get_R(seen_x, seen_y)
        y_dist, y_pred = deepsad.predict(test_x)
        print('Anomaly Detection report:')
    ...
    for i in lst:
        ...
        deepsad = SeqDeepSAD.DeepSAD(input_dim=24, emb_dim=64, out_dim=128, max_epoch=200, batch_size=1024, nu=0.99)
        deepsad.train_DeepSAD(seen_x, seen_y, unseen_x)
        deepsad.get_R(seen_x, seen_y)
        y_dist, y_pred = deepsad.predict(test_x)
        print('Anomaly Detection report:')
